...
4507246,
4526875]
collision_ids = [str(n) for n in ids]

inpath = 'C:/Users/karl/python/switrs/CCRS/'
outpath = 'C:/Users/karl/python/switrs/CCRS_inj_test/'

# Do not edit below this line --------------------------------------------------

# Helper functions

def get_crashes(crashes, crash_keys, city_name, collision_ids):
    ...
    for n in range(ncrashes):
        if crashes["City Name"][n] == city_name and crashes['Collision Id'][n] in collision_ids:
...
def run_filters(city_search,year, crashes_all, parties_all, injureds_all, crash_keys, party_keys, injured_keys):
    ...
    for city in city_search:
        ...
        crashes = get_crashes(crashes_all, crash_keys, city, collision_ids)
        print(f"\nTime to filter {city} crashes: {time.perf_counter()-begtime:.4f} sec")
        begtime = time.perf_counter()

        # now filter parties based on matched "Collision_Id"
        parties = get_parties(parties_all, party_keys, collision_ids)
        ...
        injureds = get_injureds(injureds_all, injured_keys, collision_ids)
