...
# Vista had many bizarre crashes all over the County (~3,500)
# Used geolocations (knowing somewhat imperfect) and mapping tools to cut it down with a polygon enclosure
collision_id_file = 'C:/Users/karl/python/switrs/CCRS_raw/vista_2015_good_collision_ids.csv'

# Do not edit below this line --------------------------------------------------

def run_filters(city_search,year, crashes_all, good_collision_ids, parties_all, injureds_all, crash_keys, party_keys, injured_keys):
    ...
    for city in city_search:
        ...
        # filter crashes based on City Name and collision Ids bound by city boundaries
        crashes = [crash for crash in crashes_all if crash["City Name"]==city]
        # ensure only good Collision IDs are used
        crashes = [crash for crash in crashes if crash['Collision Id'] in good_collision_ids]
...
def main():
    ...
    for year in years:
        ...
        # read in special collision IDs file
        data, keys = getDataCsv(collision_id_file, ',')
        collision_ids = data['CollisionId']
        ...
        run_filters(search_cities, year, crashes, collision_ids, parties, injureds, crash_keys, party_keys, injured_keys)
